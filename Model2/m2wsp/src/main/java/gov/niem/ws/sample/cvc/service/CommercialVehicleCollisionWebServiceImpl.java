...
public class CommercialVehicleCollisionWebServiceImpl implements CommercialVehicleCollisionPortType {
    ...
    static {
        ...
        if (DEBUG) {
            logger.log(Level.FINEST, "WSP: CommercialVehicleCollisionWebServiceImpl : initialized");
        }        
    }
    ...
    public GetDocumentResponseType getDocument(GetDocumentRequestType parameters) {
        ...
        //if for any reason we need to have an access to the assertion that user 
        //came in with here is how to get the Assertion from the Web Service Context
        ...
        if( ! GFIPMAuthorizationProvider.isUserAuthorized(currentMethodName, samlAssertion) ){
            throw new RuntimeException("User is not authorized to invoke " + currentMethodName + " method");
        }
        ...
        commercialVehicleCollisionDocument.setIncidentText(iepdOF.createIncidentText("Random text about the incident from Metro WSP"));
    ...
    public UploadPhotoResponseType uploadPhoto(UploadPhotoRequestType parameters) {
        ...
        if ((parameters.getPhoto() != null) && (parameters.getPhoto().getValue() != null)) {
            ...
            uploadPhotoResponse.setPhotoControlID(iepdOF.createPhotoControlID("WSP: Received image size is (width, height)  :  (" + data.getWidth(null) + ":" + data.getHeight(null) + ")\n" + "Assigned image id is: " + imageCounter));
            imageCounter++;
