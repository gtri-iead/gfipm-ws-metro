...
/**
 * GFIPM X509 Certificate validator. Short version of the x509 validator. Uses
 * gfipm-security-env.properties properties file. For full examples see
 * com.sun.xml.wss.impl.misc.DefaultCallbackHandler and
 * com.sun.xml.wss.impl.misc.WSITProviderSecurityEnvironment References:
...
//                    log.log(Level.INFO, "STS: 01 ");
//                }
//                if (handler == null) {
//                   handler = loadGFHandler(false, jmacHandler); 
//                   log.log(Level.INFO, "STS: 02 ");
//                }
//                
...
//        _handler = new PriviledgedHandler(handler);
    ...
    public boolean validate(X509Certificate certificate) throws CertificateValidationException {
        ...
        if (DEBUG) log.log(Level.INFO, "STS: validate: Validating certificate with Subject DN: " + certificate.getSubjectDN());
        ...
        if (certificate.getIssuerX500Principal().equals(certificate.getSubjectX500Principal())) {
            if (isTrustedSelfSigned(certificate, getTrustStore(this.runtimeProps))) {
                log.log(Level.FINEST, "STS: self-signed certificate validated in truststore");
                return true;
            } else {
                ...
                throw new CertificateValidationException("STS: Validation of self-signed certificate failed");
            }
        }

        if (DEBUG) log.log(Level.INFO, "STS: Cert is not self-signed, continuing validation");
